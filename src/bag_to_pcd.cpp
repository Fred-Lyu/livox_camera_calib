...
int main(int argc, char **argv) {
  ...
  nh.param<bool>("is_single_frame_output", is_single_frame_output, true);
  ...
  if(!is_single_frame_output){
  ROS_INFO("\033[0;31m Multi frame point cloud output\033[0m");
  ...
  ROS_INFO("\033[0;31m Single frame point cloud output\033[0m");
  std::vector<string> lidar_topic_vec;
  lidar_topic_vec.push_back(lidar_topic);
  rosbag::View view(bag, rosbag::TopicQuery(lidar_topic_vec));
  for (const rosbag::MessageInstance &m : view) {
  ...
  string msg = "Sucessfully save point cloud to pcd path: " + pcd_path;
  ROS_INFO_STREAM(msg.c_str());
  }
  return 0;
}
